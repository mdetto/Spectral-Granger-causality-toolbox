function [v]=arsim_periodic(w,omega,Ampl,Fi,n,ndisc)
...
%   Fi      = Fi(:)';    
  ...
  a=Ampl(1):(Ampl(2)-Ampl(1))/(ndisc+n-1):Ampl(2);
  f=Fi(1):(Fi(2)-Fi(1))/(ndisc+n-1):Fi(2);
  randamp = a(randperm(ndisc+n))';
  randFi  = f(randperm(ndisc+n))';
  prdc=randamp.*sin(omega*[1:ndisc+n]' + randFi);
  Wien=randn([ndisc+n,m]);
  Wien(:,1)=prdc;
